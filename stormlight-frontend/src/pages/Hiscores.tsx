...
const Hiscores = () => {
  ...
  const filteredAndSortedData = hiscoresData?.hiscores
    .filter(player => 
      player.username.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      const aXp = a.stats[sortBy]?.xp || 0
      const bXp = b.stats[sortBy]?.xp || 0
      return bXp - aXp
    }) || []
    ...
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-white mb-2">
          ...
          Stormlight Clan Hiscores
        </h1>
        <p className="text-slate-300">
          Compete with your clan mates and climb the rankings
        </p>
      </div>
      ...
      {filteredAndSortedData.length === 0 && !loading && (
        <Card className="bg-slate-800/50 border-slate-700">
          <CardContent className="p-8 text-center">
            <p className="text-slate-400">No players found matching your search criteria.</p>
          </CardContent>
        </Card>
      )}
    </div>
